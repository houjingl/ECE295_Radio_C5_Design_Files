...
// TUT global variable:
int tut_page_index = 0;
bool tut_started = false;
bool tut_ended = true;
...
int main(void) {
  ...
  while (1) {
    ...
    // User menu layers:
    // 1st: Homepage -> awaiting user inputs, prompt user to push any of the
    // three button to start inputing
    ...
    if (button1_read() || button2_read() || button3_read()) {
      ...
      // tutorial start:
      tutorial();
      while (user_input_detected && tut_ended) {
        while (layer2) {
        ...
        while (layer3) {
          ...
          if (button2_read()) {
            ...
            PLL_freq = user_confirmed_freq_Mhz * 1000000 +
                       user_confirmed_freq_Khz * 1000;
        ...
        // setup_clock(SI5351_PLL_A, SI5351_PORT0, division, 0, 1);//800 /
        // (80/3) =30 Mhz setup_clock(SI5351_PLL_A, SI5351_PORT1, division, 0,
        // 1);//800 / (80/3) =30 Mhz
...
void tutorial() {
  ...
  }  // else {// tut ended or not started or button pressed}
  layer2 = true;
}
// TIMER ISR:
// ISR(TIMER0_COMPA) {
//   if (tut_started && (!tut_ended)) {
//     tut_page_index = tut_page_index + 1;
//     if (tut_page_index == 0) {  // page 0
//       tut_started = false;
//       tut_ended = false;
//     } else if (tut_page_index == 4) {  //--> page 4 = layer 2 begin
//       tut_started = true;
//       tut_ended = true;
//     } else { // page 1 2 3
//       tut_started = true;
