  ...
  while (1) {
    switch (current_state) {
      ...
      case STATE_LAYER2_STAGE1:  // Layer2: handle TX/RX MODE. Stage1: SELECT
        ...
        if (button2_read()) {  // confirm the selected mode
          current_state = STATE_LAYER2_STAGE2;
          LCD_Clear_screen();
          break;
        }

        if (knobL_read()) {  // go back to wait state to select comp/user
          current_state = STATE_WAIT;
          LCD_Clear_screen();
          break;
        }

        if (knobR_read()) {  // go to adjust frequency(PLL)
      ...
      case STATE_LAYER2_STAGE2:  // Layer2: handle TX/RX MODE. Stage2: CONFIRM
        ...
        current_state = STATE_CONFIG_TXEN;
        break;
      ...
      case STATE_LAYER3:  // Layer3: frequency adjustment
        ...
        PLL_freq = user_confirmed_freq_Mhz * 10 +
                   (user_confirmed_freq_Khz /
                    100);  // Accurate to with in .1 decimal places. Limited by
                           // MCU computating power
        // show confirmed frequency:
        ...
        current_state = STATE_CONFIG_PLL;
        // }
      ...
      case STATE_CONFIG_PLL:  // configure PLL and TXEN.
      ...
      case STATE_CONFIG_TXEN:  // configure PLL and TXEN.
